...
class ExpenseTracker(object):
    ...
    def edit_expense(self) -> None:
        ...
        if self.data and self.data['expenses']:
            for idx, expense in enumerate(self.data['expenses']):
                if idx == id:
                    print('\nPress `Enter` to use default entry!')
                    expense['name']: str = input('\tEnter new name: ')
                    expense['category']: str = input('\tEnter the category: ').strip().title()
                    expense['amount']: str = f'${float(input('\tEnter the amount: ').strip()):.2f}'
    ...
    def delete_expense(self) -> None:
        ...
        if self.data and self.data['expenses']:
            for idx, expense in enumerate(self.data['expenses']):
                self.data['expenses'].remove(expense) if idx == id else None
            print('Expense removed successfully!\n')

    # DONE: Expense Categories: categorize expenses and summarize expenses by category
    def filter_by_category(self) -> None:
        if self.data and self.data['expenses']:
            categories: list[str] = [_category for _category in self.data['expenses']['category']]
    ...
    def filter_by_date_range(self, callback: str | None = None) -> None:
        if self.data and self.data['expenses']:
    ...
    # TODO: Total spending: Calculate and display the total spending over a specific period
    def total_spending(self) -> None:
        start_date, end_date, filtered_expenses: list[Mapping[str, str]] = self.filter_by_date_range(callback=not(None))
        total_expenses: float = float()

        for expense in filtered_expenses:
            total_expenses += float(expense['amount'].replace('$', ''))
        
        print(f'Total Expenses between {start_date} and {end_date} is: ${total_expenses}')
    ...
    def monthly_budget(self) -> None:
        if self.data and self.data['expenses']:
            ...
            print(f'Monthly Budget Deficit Amount: ${turnover}') if turnover < 0 else print(f'Monthly Budget Surplus Amount: ${turnover}') if turnover > 0 else print('Monthly Budget Has Broken Even!')
